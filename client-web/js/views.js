...
messages = data.messages ?? [];
    ...
    async function tryDecrypt(msg) {
        ...
        try {
            ...
            if (!senderKeyCache[senderUsername]) {
                const senderUser = await api.getUser(senderUsername);
                if (!senderUser?.x25519_public_key) return '(encrypted)';
                const senderKeyBytes = Uint8Array.from(atob(senderUser.x25519_public_key), c => c.charCodeAt(0));
                senderKeyCache[senderUsername] = await crypto.subtle.importKey(
                    'raw', senderKeyBytes, { name: 'X25519' }, false, ['deriveBits'],
                );
            }
            const senderStaticPubKey = senderKeyCache[senderUsername];
    ...
    try {
        switch (action) {
            ...
            case 'forward': {
                ...
                const origEphPkBytes = hexToBytes(orig.ephemeral_pk);
